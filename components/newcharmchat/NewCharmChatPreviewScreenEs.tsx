...
function extractMessagesFromFlat(data: Record<string, string>) {
  const messages = []
  let i = 1
  while (data[`No Digas Mensaje ${i}`]) {
    messages.push({
      text: data[`No Digas Mensaje ${i}`],
      description: data[`Di Mensaje ${i}`] || '',
    })
    i++
  }
  return messages
...
/* ---------------------------- 🔥 MAIN COMPONENT --------------------------- */
export default function NewCharmChatPreviewScreenEs({ images }: newCharmChatPreviewScreenProps) {
  const data = useNewCharmChatStore(state => state.data);
  const title = data?.['Título'] ?? ''
  const subtitle = addArrowSuffix(data?.['Subtítulo'] ?? '')
  const tone = data?.['Tono'] ?? ''
  const reply = data?.['Prompt de Mensaje'] ?? ''
...
// Hook to properly cleanup object URLs
function useObjectURL(file: File | string | undefined): string {
  const url = useMemo(() => {
  ...
  useEffect(() => {
    return () => {
      if (file && typeof file !== 'string' && url.startsWith('blob:')) {
